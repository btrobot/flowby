...
class Lexer:
    ...
    def _scan_token(self):
        ...
        # 未知字符
        raise LexerError(self.line, self.column, f"未知字符: {char!r}", self._get_current_line())
    ...
    def _scan_string(self):
        ...
        if self._is_at_end():
            raise LexerError(
                start_line, start_column, "字符串未结束（遇到文件结尾）", self._get_current_line()
            )
    ...
    def _skip_triple_quote_comment(self):
        ...
        # 未找到结束标记
        raise LexerError(
            start_line, start_column, '未闭合的三引号注释：缺少 """', self._get_current_line()
        )
    ...
    def _scan_identifier(self):
        ...
        if value.lower() in ("true", "false", "null", "none"):
            ...
            # 获取正确的 Python 风格写法
            correct_form = {"true": "True", "false": "False", "null": "None", "none": "None"}.get(
                value.lower(), value
            )
