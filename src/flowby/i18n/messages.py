...
MESSAGES: Dict[str, Dict[str, str]] = {
    ...
    'lexer.unterminated_fstring': {
        'zh': "未闭合的 f-string",
        'en': "Unterminated f-string"
    },
    ...
    'exec.timeout': {
        'zh': "操作超时（{timeout}秒）",
        'en': "Operation timeout ({timeout}s)"
    },

    'exec.assertion_failed': {
        'zh': "断言失败: {condition}",
        'en': "Assertion failed: {condition}"
    },
    ...
    'exec.division_by_zero': {
        'zh': "除数不能为零",
        'en': "Division by zero"
    },

    'exec.index_out_of_range': {
        'zh': "索引超出范围: {index}",
        'en': "Index out of range: {index}"
    },

    'exec.undefined_function': {
        'zh': "未定义的函数: {name}",
