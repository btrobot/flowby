...
class Parser:
    ...
    def _parse_resource(self) -> 'ResourceStatement':
        """
        解析 resource 语句（v4.2+: OpenAPI 规范导入，v3.0风格缩进）
        ...
        # 检查名称冲突
        if self.symbol_table_stack.exists_in_current_scope(name):
            current_token = self._peek()
            raise ParserError(
                line=current_token.line,
                column=current_token.column,
                token_type=str(current_token.type),
                token_value=str(current_token.value),
                message=f"名称 '{name}' 已被使用"
            )
            ...
            while not self._check(TokenType.DEDENT):
                if self._is_at_end():
                    ...
                    raise ParserError(
                        ...
                        token_type=str(current_token.type),
                        token_value=str(current_token.value),
                        message="resource 块未正确结束"
                    )

                # 解析配置项
                config_key = self._consume(TokenType.IDENTIFIER, "期望配置项名称").value
                self._consume(TokenType.COLON, "期望 ':'")

                if config_key == "spec":
                    spec_file = self._consume(TokenType.STRING, "期望字符串").value
                    ...
                    timeout = int(self._consume(TokenType.INTEGER, "期望整数").value)
                    ...
                    raise ParserError(
                        ...
                        token_type=str(current_token.type),
                        token_value=str(current_token.value),
                        message=f"未知的配置项 '{config_key}'"
                    )

                self._consume(TokenType.NEWLINE, "期望换行")

            # v3.0: 只用 DEDENT 结束块，不需要额外的 'resource' 标记（统一Python风格）
            self._consume(TokenType.DEDENT, "期望结束缩进")
