...
def migrate_file(file_path: Path, dry_run: bool = False) -> bool:
    ...
    # 检查是否包含 v2.0 语法
    has_v2_syntax = (
        bool(re.search(r"\bend\s+(if|step|for|while|when|function)\b", original_content))
        or bool(re.search(r"\b(false|true|null)\b", original_content))
        or bool(re.search(r"\$context|\$page|\$env", original_content))
    )
