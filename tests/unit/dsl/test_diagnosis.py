...
from registration_system.dsl.diagnosis import DiagnosisLevel, DiagnosisConfig, DEFAULT_DIAGNOSIS_CONFIG
...
from registration_system.dsl.diagnosis.collectors import (
    ...
    ScreenshotCollector, PageInfoCollector, HtmlSourceCollector,
    ElementInfoCollector, ConsoleLogCollector, ContextSnapshotCollector,
    NetworkLogCollector, PerformanceCollector, ViewportCollector
)
...
class TestDiagnosisLevelEnum:
    ...
    def test_diagnosis_level_values(self):
        """测试诊断级别枚举值"""
        # Arrange & Act & Assert
        assert DiagnosisLevel.NONE == 0, \
            "NONE 级别应该是 0"
        assert DiagnosisLevel.MINIMAL == 1, \
            "MINIMAL 级别应该是 1"
        assert DiagnosisLevel.BASIC == 2, \
            "BASIC 级别应该是 2"
        assert DiagnosisLevel.STANDARD == 3, \
            "STANDARD 级别应该是 3"
        assert DiagnosisLevel.DETAILED == 4, \
            "DETAILED 级别应该是 4"
        assert DiagnosisLevel.FULL == 5, \
            "FULL 级别应该是 5"

    def test_from_string_lowercase(self):
        """测试从小写字符串解析级别"""
        # Arrange & Act & Assert
        assert DiagnosisLevel.from_string("none") == DiagnosisLevel.NONE, \
            "none 应该解析为 NONE"
        assert DiagnosisLevel.from_string("minimal") == DiagnosisLevel.MINIMAL, \
            "minimal 应该解析为 MINIMAL"
        assert DiagnosisLevel.from_string("basic") == DiagnosisLevel.BASIC, \
            "basic 应该解析为 BASIC"
        assert DiagnosisLevel.from_string("standard") == DiagnosisLevel.STANDARD, \
            "standard 应该解析为 STANDARD"
        assert DiagnosisLevel.from_string("detailed") == DiagnosisLevel.DETAILED, \
            "detailed 应该解析为 DETAILED"
        assert DiagnosisLevel.from_string("full") == DiagnosisLevel.FULL, \
            "full 应该解析为 FULL"
...
class TestDiagnosisConfig:
    ...
    def test_default_config_level(self):
        ...
        # Act & Assert
        assert config.default_level == DiagnosisLevel.STANDARD, \
            "默认级别应该是 STANDARD"
    ...
    def test_error_type_level_mapping_assertion(self):
        ...
        # Assert
        assert level == DiagnosisLevel.STANDARD, \
            "ASSERTION_FAILED 应该使用 STANDARD 级别"
    ...
    def test_error_type_level_mapping_element(self):
        ...
        # Assert
        assert level == DiagnosisLevel.BASIC, \
            "ELEMENT_NOT_FOUND 应该使用 BASIC 级别"
    ...
    def test_error_type_level_mapping_timeout(self):
        ...
        # Assert
        assert level == DiagnosisLevel.DETAILED, \
            "TIMEOUT 应该使用 DETAILED 级别"
    ...
    def test_cleanup_config(self):
        ...
        # Act & Assert
        assert config.cleanup.enabled == True, \
            "清理应该默认启用"
        assert config.cleanup.max_age_days == 7, \
            "最大保留天数应该是 7"
        assert config.cleanup.max_count == 100, \
            "最大保留数量应该是 100"
        assert config.cleanup.max_size_mb == 500, \
            "最大保留大小应该是 500MB"
    ...
    def test_network_filter_config(self):
        ...
        # Act & Assert
        assert config.network_filter.include_assets == False, \
            "默认不包含静态资源"
        assert config.network_filter.only_failed == False, \
            "默认不仅记录失败请求"
    ...
    def test_console_filter_config(self):
        ...
        # Act & Assert
        assert "error" in config.console_filter.levels, \
            "应该包含 error 级别"
        assert "warning" in config.console_filter.levels, \
            "应该包含 warning 级别"
        assert config.console_filter.max_entries == 100, \
            "最大条目数应该是 100"

    @pytest.mark.parametrize("error_type,expected_level", [
...
class TestCollectorsForLevel:
    ...
    def test_minimal_level_collectors(self):
        ...
        # Assert
        assert 'screenshot' in collectors, \
            "MINIMAL 应该包含 screenshot"
        assert 'page_info' in collectors, \
            "MINIMAL 应该包含 page_info"
    ...
    def test_basic_level_collectors(self):
        ...
        # Assert
        assert 'html_source' in collectors, \
            "BASIC 应该包含 html_source"
        assert 'element_info' in collectors, \
            "BASIC 应该包含 element_info"
    ...
    def test_standard_level_collectors(self):
        ...
        # Assert
        assert 'console_logs' in collectors, \
            "STANDARD 应该包含 console_logs"
        assert 'context_snapshot' in collectors, \
            "STANDARD 应该包含 context_snapshot"
    ...
    def test_detailed_level_collectors(self):
        ...
        # Assert
        assert 'network_logs' in collectors, \
            "DETAILED 应该包含 network_logs"
    ...
    def test_full_level_collectors(self):
        ...
        # Assert
        assert 'performance_metrics' in collectors, \
            "FULL 应该包含 performance_metrics"
        assert 'viewport_info' in collectors, \
            "FULL 应该包含 viewport_info"
...
class TestCollectorsRegistry:
    ...
    def test_collector_registered(self, collector_name):
        ...
        assert collector is not None, \
            f"收集器 {collector_name} 应该已注册"
        assert collector.name == collector_name, \
            f"收集器名称应该是 {collector_name}"
    ...
    def test_all_collectors_have_name(self):
        ...
        # Arrange
        expected_collectors = [
            'screenshot', 'page_info', 'html_source', 'element_info',
            'console_logs', 'context_snapshot', 'network_logs',
            'performance_metrics', 'viewport_info'
        ]
...
class TestLexerDiagnosisTokens:
    ...
    def test_diagnosis_token(self, lexer):
        ...
        # Assert
        assert tokens[0].type == TokenType.DIAGNOSIS, \
            "diagnosis 应该生成 DIAGNOSIS token"
...
class TestParserDiagnosisSyntax:
    ...
    @pytest.mark.parametrize("code,expected_type,expected_level", [
        ...
        ('assert $element.text(".selector") contains "hello" with diagnosis basic', 'expression', 'basic'),
        ('assert $element.value("#input") equals "x" with diagnosis detailed', 'expression', 'detailed'),
    ...
    def test_assert_with_diagnosis_level(
        self,
        lexer,
        parser,
        code,
        ...
        assert isinstance(stmt, AssertStatement), \
            "应该解析为 AssertStatement"
        assert stmt.assert_type == expected_type, \
            f"断言类型应该是 {expected_type}"
        assert stmt.diagnosis_level == expected_level, \
            f"诊断级别应该是 {expected_level}"
    ...
    def test_assert_without_diagnosis_level(self, lexer, parser):
        ...
        # Assert
        assert stmt.diagnosis_level is None, \
            "没有诊断级别时应该是 None"
...
class TestReportGenerator:
    ...
    def test_report_contains_error_info(self, generator, test_data):
        ...
        # Assert
        assert "ASSERTION_FAILED" in report, \
            "报告应该包含错误类型"
        assert "URL 断言失败" in report, \
            "报告应该包含错误消息"
    ...
    def test_report_has_all_sections(self, generator, test_data):
        ...
        # Assert
        assert "## 错误摘要" in report, \
            "报告应该有错误摘要章节"
        assert "## 页面信息" in report, \
            "报告应该有页面信息章节"
        assert "## 元素信息" in report, \
            "报告应该有元素信息章节"
        assert "## 控制台日志" in report, \
            "报告应该有控制台日志章节"
        assert "## 诊断文件" in report, \
            "报告应该有诊断文件章节"
...
class TestIntegration:
    ...
    def test_execution_context_has_diagnosis_manager(self):
        ...
        # Assert
        assert ctx.diagnosis_manager is not None, \
            "ExecutionContext 应该有 diagnosis_manager"
    ...
    def test_execution_context_has_diagnosis_config(self):
        ...
        # Assert
        assert ctx.diagnosis_config is not None, \
            "ExecutionContext 应该有 diagnosis_config"
        assert ctx.diagnosis_config.default_level == DiagnosisLevel.STANDARD, \
            "默认级别应该是 STANDARD"
    ...
    def test_execution_context_diagnosis_listeners_none(self):
        ...
        # Assert
        assert ctx.diagnosis_listeners is None, \
            "DiagnosisListeners 初始应该为 None"
    ...
    def test_set_diagnosis_config(self):
        ...
        # Assert
        assert ctx.diagnosis_config.default_level == DiagnosisLevel.FULL, \
            "设置的配置应该生效"
...
'''
        ...
        # Assert
        assert asserts[0].diagnosis_level == "full", \
            "第一个断言应该是 full 级别"
        assert asserts[1].diagnosis_level == "standard", \
            "第二个断言应该是 standard 级别"
        assert asserts[2].diagnosis_level == "minimal", \
            "第三个断言应该是 minimal 级别"
