...
class TestBasicConfigLoading:
    ...
    def test_load_empty_services(self, temp_dir, loader):
        ...
        assert isinstance(config, ServicesConfig), \
            "应该返回 ServicesConfig 实例"
        assert config.settings.timeout == 30000, \
            "默认 timeout 应该是 30000"
        assert config.settings.retry_count == 3, \
            "默认 retry_count 应该是 3"
...
""")
        ...
        # Assert
        assert config.settings.timeout == 60000, \
            "timeout 应该是 60000"
        assert config.settings.retry_count == 5, \
            "retry_count 应该是 5"
        assert config.settings.retry_delay == 2000, \
            "retry_delay 应该是 2000"
...
""")
        ...
        # Assert
        assert "random" in config.providers, \
            "应该包含 random 提供者"
        assert config.providers["random"].type == "random", \
            "提供者类型应该是 random"
        assert config.providers["random"].config["charset"] == "abc", \
            "配置应该包含 charset"
...
""")
        ...
        assert config.providers["random"].type == "random", \
            "random 提供者类型应该正确"
        assert config.providers["email"].type == "tempmail", \
            "email 提供者类型应该是 tempmail"
        assert config.providers["email"].timeout == 60000, \
            "email 提供者应该有自定义 timeout"
...
""")
        ...
        try:
            ...
            # Assert
            assert config.providers["api"].config["api_key"] == "secret123", \
                "环境变量应该被正确解析"
        finally:
...
""")
        ...
        try:
            ...
            # Assert
            assert config.providers["api"].config["token"] == "token456", \
...
""")
        ...
        # Assert
        assert config.providers["api"].config["key"] == "default_value", \
            "应该使用默认值"
...
""")
        ...
        try:
            ...
            # Assert
            assert config.providers["api"].config["url"] == "http://localhost:8080", \
                "多个环境变量应该都被正确替换"
        finally:
            del os.environ["HOST"]
            del os.environ["PORT"]
    ...
    def test_various_env_var_values(self, temp_dir, loader, env_var, value, expected):
        ...
        self._write_config(temp_dir, "services.yaml", f"""
providers:
  api:
    type: "http"
    config:
...
""")
    ...
    def test_various_invalid_settings(
        self,
        temp_dir,
        loader,
        invalid_config,
...
""")
        ...
        # Assert
        assert variables["user"]["first_name"] == "John", \
            "first_name 应该是 John"
        assert variables["user"]["last_name"] == "Doe", \
            "last_name 应该是 Doe"
...
""")
        ...
        try:
            ...
            # Assert
            assert variables["user"]["email"] == "test@example.com", \
                "环境变量应该被正确解析"
        finally:
    ...
    def test_load_empty_variables(self, temp_dir, loader):
        ...
        # Assert
        assert variables == {} or variables is None, \
            "空文件应该返回空字典或 None"
    ...
    def test_various_variable_types(self, temp_dir, loader, var_name, var_value):
        ...
        # Assert
        assert variables["test"][var_name] == var_value, \
            f"{var_name} 应该是 {var_value}"
...
class TestConfigErrorFormatting:
    ...
    def test_error_with_location(self):
        ...
        # Assert
        message = str(error)
        assert "第 10 行" in message, \
            "错误消息应该包含行号"
        assert "第 5 列" in message, \
            "错误消息应该包含列号"
    ...
    @pytest.mark.parametrize("message,file_path,line,column", [
        ("错误1", "file1.yaml", 1, 1),
        ("错误2", "file2.yaml", 10, 5),
